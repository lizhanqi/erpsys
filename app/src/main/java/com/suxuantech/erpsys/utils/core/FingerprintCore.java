package com.suxuantech.erpsys.utils.core;
...
public class FingerprintCore {
    ...
    public FingerprintCore(Context context) {
        ...
        L.d("fingerprint isSupport: " + isSupport);
        initCryptoObject();
    }
    ...
    private void initCryptoObject() {
        try {
            ...
            L.d("create cryptoObject failed!");
        }
    }
    ...
    private void notifyAuthenticationError(int errMsgId, CharSequence errString) {
        L.d("onAuthenticationError, errId:" + errMsgId + ", err:" + errString + ", retry after 30 seconds");
        if (null != mFpResultListener && null != mFpResultListener.get()) {
    ...
    private void notifyAuthenticationFailed(int msgId, String errString) {
        L.d("onAuthenticationFailed, msdId: " +  msgId + " errString: " + errString);
        if (null != mFpResultListener && null != mFpResultListener.get()) {
    ...
    private void onFailedRetry(int msgId) {
        mFailedTimes++;
        L.d("on failed retry time " + mFailedTimes);
        cancelAuthenticate();
    ...
    public static FingerprintManager getFingerprintManager(Context context) {
        ...
        try {
            ...
            L.d("have not class FingerprintManager");
        }
